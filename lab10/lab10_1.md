...
```r
options(scipen = 999) #cancels scientific notation for the session, here is doesn't do much for us bc we need to use log 10 of the mass
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% ggplot(aes(x=order,y=mean_mass))+geom_col()+coord_flip()+scale_y_log10()#here we scale the y axis by log 10
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% ggplot(aes(x=reorder(order,mean_mass),y=mean_mass))+geom_col()+coord_flip()+scale_y_log10() #reorder function within aes(), but must also include y in ().
```
...
```r
life_history %>% 
  ggplot(aes(x=gestation,y=wean_mass))+geom_point(na.rm=T)+scale_y_log10() #using reorder can improve this more
```
...
```r
elephants %>% 
  ggplot(aes(x=sex,fill=sex))+geom_bar() #fill() colors the bars and makes a key 
```
