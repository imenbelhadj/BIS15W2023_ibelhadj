...
```{r}
display.brewer.pal(4,"Paired") #this is from the ^website, select color blind safe, can use same pallete for whole report
```
...
```{r}
homerange %>% 
  ggplot(aes(x=trophic.guild,fill=class))+geom_bar(position=dodge)+scale_fill_manual()
```
...
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+ 
  geom_boxplot(alpha=0.4)+ 
  facet_grid(migratory_strategy~.)+ # . means the column variable isn't specified, row name comes before the ~
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  labs(title = "Mass by Diet and Migratory Strategy",
       x = NULL,
       y = "Log 10 Mass",
...
## Practice
1. Build a faceted plot that shows a distribution of log10 mass by habitat type.
```{r}
ecosphere %>% 
  ggplot(aes(x=log10_mass))+ 
  geom_density()+ 
