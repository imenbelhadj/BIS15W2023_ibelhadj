...
```{r}
names(penguins)  #first, select the continuous variables
```

Since we will make a scatterplot, let's focus on the continuous variables. The first step is to setup the x variable input.
```{r, eval=F, echo=T}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm")   #these are the continuous variables
)      #selecected refers to where the dropdown menu starts
...
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm"),  #these are the continuous variables
      #selected refers to where the dropdown menu starts
...
server <- function(input, output) {
  reactive({input$x
            input$y})   #looks for x and y from above, still have to tell app what to do
}
...
server <- function(input, output) {
  output$plot<- renderPlot({
    ggplot(data = penguins, aes_string(x=input$x, y=input$y, color="species"))+geom_point()
  })
  }
  #reactive({input$x
        #    input$y})   #looks for x and y from above, still have to tell app what to do
...
```{r}
homerange %>% 
  ggplot(aes(x=locomotion, fill=trophic.guild))+geom_bar(position="dodge")
```

A little nicer aesthetics...  
```{r}
homerange %>% 
  ggplot(aes(x=locomotion, fill=thermoregulation))+geom_bar(position="dodge")
```
...
server <- function(input, output, session) {
  output$plot <-renderPlot({
    ggplot(data=homerange,aes_string(x="locomotion", fill=input$x)) +
      geom_bar(position = "dodge", alpha=0.8) + labs(x=NULL, y=NULL, fill="Fill Variable")
  })
