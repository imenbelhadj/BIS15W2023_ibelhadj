...
files   #tells all files in a certain path
```
...
```{r, message = FALSE}
spider_list <- lapply(files, read_csv)  #makes new list, use lapply function telling it to apply read_csv function to files
```
...
table_B
```
Merge the 2 data sets bc one isn't in Table A and one isn't in Table B.
...
inner_exampleDF
```
Only kept customers in common between the 2 datasets.
`left_join`
...
left_exampleDF
```
Only took customers in first dataset
`right_join`
...
right_exampleDF
```
Only took customers in 2nd dataset.
`full_join`
```{r}
full_exampleDF <- full_join(table_A, table_B, by="customer_ID")
full_exampleDF   #table A doesn't have data for customer 4 
```
Merged all customers into one regardless of which they were/weren't in before. 
`anti_join`
Provides the rows in the first table for which there are not matching values in the second table.
```{r}
anti_exampleDF <- anti_join(table_A, table_B, by="customer_ID")
anti_exampleDF #telling you which rows data would be lost if you did full_join
```
