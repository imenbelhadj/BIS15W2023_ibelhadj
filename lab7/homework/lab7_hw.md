...
```r
amniota %>% summarise(number_nas=sum(is.na(amniota)))
```
...
```r
amphibio %>% summarise(number_nas=sum(is.na(amphibio)))
```
...
amniota_tidy %>% 
  group_by(class) %>%
  summarise(mean_egg_mass=mean(egg_mass_g,na.rm = T),num_na=sum(is.na(egg_mass_g)))
```

```
## # A tibble: 3 × 3
##   class    mean_egg_mass num_na
##   <chr>            <dbl>  <int>
## 1 Aves              22.0   4914
## 2 Mammalia         NaN     4953
## 3 Reptilia          24.3   6040
```

```r
#Since mammals don't lay eggs, it's expected that there would be as many NAs for egg mass as the number of mammals. NAs are also used when specific data doesn't apply to that row.
```
