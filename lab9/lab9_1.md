...
```r
#overplotting is commmon in big data->points overlap, use geom_jitter to ass extra space in plot
```

To add a regression (best of fit) line, we just add another layer.

```r
#ggplot(homerange,mapping = #aes(x=log10.mass,y=log10.hra))+geom_point()+geom_()
```
...
```r
homerange %>% 
  ggplot(aes(x=trophic.guild)) + #don't have to specify a y here
...
homerange %>% 
  ...
  count(trophic.guild) %>% 
  ggplot(aes(x=trophic.guild,y=n))+ #don't have to use mapping with pipes
  geom_col()
```
...
#  filter(class=="mammalia") %>% 
#  top_n(-10,log10.mass) %>% 
#  ggplot(aes(x=common.name,y=log10.mass))+ #don't have to use mapping with #pipes
#  geom_col()+#count(flip)
```
